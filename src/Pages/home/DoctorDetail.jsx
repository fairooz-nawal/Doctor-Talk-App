...
const DoctorDetail = () => {
    const { doctor, handleSetBooking, getCartFromLocalStorage,validId,handleInvalid } = useContext(ApiProvider)
    const { id } = useParams();
    const intId = parseInt(id);
    const single = doctor.find(singleDoc => singleDoc.id == intId)
    
    const singleValid = validId.find(validSingleId => validSingleId == intId);
        ...
        return (
            <div className="max-w-full lg:max-w-7xl mx-auto px-5 lg:px-[100px] pb-[80px]">
                ...
                <div className="w-full mx-auto text-center bg-white p-[50px] my-10 rounded-2xl">
                    ...
                    <button onClick={() => handleNavigate(single.id,single.name)} className="w-full my-2 p-2 text-white bg-blue-700  rounded-2xl hover:bg-blue-900">
